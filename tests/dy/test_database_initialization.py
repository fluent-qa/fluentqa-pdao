...
from tests.dy import mock_create_engine_fixture, setup_mock_engine
...
def fixture_mock_engine(mock_create_engine):
    ...
    if databases.is_set_current_database_supported():
        databases.reset_current_database()
...
def fixture_reset_init_hook():
    yield
    if hasattr(databases.Database, "hook_method"):
        delattr(databases.Database, "hook_method")
...
def test_init_hook_multiple_databases(mock_engine):
    ...
    mock_engine.connect().execution_options().execute.return_value = []
    query()
    databases.set_current_database("d2")
    query()
...
def test_current_database_default(mock_engine, mock_create_engine):
    db_container = databases.DatabaseContainerSingleton()
...
def test_is_set_current_database_supported():
    ...
    if "3.6" in sys.version:
        assert not databases.is_set_current_database_supported()
    else:
        assert databases.is_set_current_database_supported()
...
def test_current_database_set(mock_engine, mock_create_engine):
    db_container = databases.DatabaseContainerSingleton()
    databases.set_current_database("db1")
    mock_engine.connect().execution_options().execute.return_value = []
    query()
